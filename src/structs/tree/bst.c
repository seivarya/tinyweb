...
static inline void _validate_bst_node_construction(bst *tree, bst_node *node) {
  if (!node) {
    fprintf(stderr, "Error: %s: Failed to construct BST node, aborting.\n",
            __func__);
    /* tree may be partially built at this point; caller must decide how to
     * recover */
    exit(3);
  }
}
...
void *bst_search(bst *tree, void *data) {
  ...
  if (direction == 0) {
    /* INFO: the data is actually a bst_node itself. in dict it'll be a node
     * with <entry> wrapper over it. */
    return cursor->data;
  } else {
