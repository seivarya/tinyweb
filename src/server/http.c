...
static char *static_file_response(const char *status, const char *filepath) {
        ...
        size_t body_size = (size_t)file_len;
        ...
        size_t read_sz = fread(body, 1, body_size, fp);
        fclose(fp);
        if (read_sz != body_size) {
                free(body);
                return NULL;
        }
        body[body_size] = '\0';
        ...
        char *response = malloc(body_size + 512);
        if (!response) {
                free(body);
                return NULL;
        }

        int hdr_len = sprintf(response,
                              "HTTP/1.1 %s\r\n"
                              "Content-Type: %s\r\n"
                              "Content-Length: %zu\r\n"
                              "Connection: keep-alive\r\n"
                              "\r\n",
                              status ? status : "200 OK", mime, body_size);
        memcpy(response + (size_t)hdr_len, body, body_size);
        response[(int)hdr_len + (int)body_size] = '\0';
        free(body);
        return response;
}
...
http *http_construct(unsigned short port, size_t thread_cnt) {
        ...
        if (!http_server) {
                fprintf(stderr,
                        "[ERROR]: %s :: malloc failed for http server struct\n",
                        __func__);
                return NULL;
        }
        ...
        http_server->srv =
            server_construct(AF_INET, SOCK_STREAM, 0, port, 255, INADDR_ANY);

        http_server->routes = dict_construct();
        strcpy(http_server->static_dir,
               "frontend"); // "frontend" itself is just a temp val
...
void register_route(http *srv, char *(*route_func)(http *srv, request *req),
        ...
        for (int i = 0; i < count_methods; i++) {
                rte->methods[i] =
                    va_arg(methods, int); // storing methods in ->methods[i]
        }
...
void launch(struct http *http_server) {
        ...
        while (1) {
                ...
                if (!cnt) {
                        fprintf(
                            stderr,
                            "[ERROR]: %s :: malloc failed for client struct\n",
                            __func__);
                        continue;
                }

                cnt->client = (int)accept(http_server->srv->socket, address,
                                          &address_length);
...
char *render_func(const char *status, int file_count, ...) {
        if (file_count <= 0) {
                fprintf(stderr,
                        "[ERROR]: %s :: file_count must be greater than zero\n",
                        __func__);
                return NULL;
        }
...
void handler(void *arg) {
        if (!arg) {
                fprintf(stderr,
                        "[ERROR]: %s :: handler received null argument\n",
                        __func__);
                return;
        }
        ...
        while (keep_alive) {
                ...
                if (!reqstr) {
                        fprintf(
                            stderr,
                            "[ERROR]: %s :: malloc failed for request buffer\n",
                            __func__);
                        break;
                }
                ...
                if (!rte) {
                        ...
                        if (!response) {
                                ...
                                snprintf(error_path, sizeof(error_path),
                                         ...
                                         cnt.server->static_dir);
                                response =
                                    render_func("404 Not Found", 1, error_path);
                        }

                        if (response) {
                                if (write(cnt.client, response,
                                          strlen(response)) < 0) {
                                        fprintf(
                                            stderr,
                                            "[ERROR]: %s :: failed to write "
                ...
                if (!response) {
                        ...
                        if (!fp) {
                                fprintf(stderr,
                                        "[ERROR]: %s :: route returned null, "
                                        "failed to open fallback index.html\n",
                                        __func__);
                        ...
                        if (!file_buf) {
                                fprintf(stderr,
                                        "[ERROR]: %s :: malloc failed for file "
                                        "buffer\n",
                                        __func__);
                                fclose(fp);
                                if (write(cnt.client, ERR_500_RESPONSE,
                ...
                if (write(cnt.client, response, strlen(response)) < 0) {
                        fprintf(stderr,
                                "[ERROR]: %s :: failed to write response to "
                                "client\n",
                                __func__);
                        keep_alive = 0;
                }
