...
void *exec_worker(void *arg) {
	if (!arg) {
		fprintf(stderr,
				"[ERROR]: %s :: worker received null argument\n", __func__);
		return NULL;
	}
	executor *exec = arg;
...
bool exec_add_work(executor *exec, exec_func func, void *arg) {
	if (!exec || !func) {
		fprintf(stderr, "[ERROR]: %s :: null executor or function pointer\n", __func__);
		return false;
	}
	pthread_mutex_lock(&(exec->mutex));
...
executor *exec_create(size_t num) {
	if (num < 10)
		num = 10; // minimun 10 threads

	executor *exec = malloc(sizeof(executor));
	if (exec == NULL) {
		fprintf(stderr, "[ERROR]: %s :: malloc failed for executor struct\n", __func__);
		return NULL;
	}
	...
	for (size_t i = 0; i < num; i++) {
		...
		if (pthread_create(&thread, NULL, exec_worker, exec) != 0) {
			fprintf(stderr, "[ERROR]: %s :: pthread_create failed at index %zu\n", __func__, i);
			pthread_mutex_lock(&(exec->mutex));
	...
	fprintf(stdout, "[SUCCESS]: %s :: executor constructed with %zu threads\n", __func__, num);
	return exec;
}

int exec_destroy(executor *exec) {
	if (!exec) {
		fprintf(stderr, "[ERROR]: %s :: null executor pointer\n", __func__);
		return 1;
	}
	...
	fprintf(stdout, "[DEBUG]: %s :: executor destructed\n", __func__);
...
int exec_wait(executor *exec) {
	...
	pthread_mutex_lock(&(exec->mutex));
	while (exec->pending_count > 0 ||
			exec->working_count > 0) { /* while work exists */
...
exec_work *exec_work_create(exec_func func, void *arg) {
	if (!func) {
		fprintf(stderr, "[ERROR]: %s :: null function pointer\n", __func__);
		return NULL;
	}
...
int exec_work_destroy(exec_work *work) {
	if (work == NULL) {
		fprintf(stderr, "[ERROR]: %s :: null work pointer\n", __func__);
		return 1;
	}
	free(work);
	return 0;
